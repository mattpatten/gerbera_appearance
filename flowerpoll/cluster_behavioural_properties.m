function cluster_behavioural_properties(imgAbbr, numClusters, componentThreshold)
...
clusterDir = sprintf('%sbehav_clusters_%igroups_%02gthr_%s%s', outputDir, numClusters, componentThreshold, datestr(now,'yyyymmdd_HHMMSS'), filesep); %folder name
...
%cut down to behavioural properties only
header = header((end-5):end);
imgProperties = imgProperties(:,(end-5):end);
...
saveas(f,[clusterDir 'PCA_details.png']);
close;

numPlaces = 10;
for i=1:length(header)
    create_examplar(imgAbbr, labels, data(:,i), ['Component ' num2str(i)], numPlaces);
end
